...
func runCmdWithPipe() {
	...
	cmd2.Stdin = &outputBuf1 // cmd1的输出作为cmd2的输入
...
func runCmd() {
	...
	stdout0, err := cmd0.StdoutPipe() // 返回一个输出管道
	if err != nil {
	...
	if !useBufferedIO {     // 使用 IO 缓冲时
		var outputBuf0 bytes.Buffer
		for {
			tempOutput := make([]byte, 5)
			n, err := stdout0.Read(tempOutput) // Read 方法把读出的输出数据存入调用方传递给它的字节切片，n为实际输出的字节数量   
			if err != nil {
				if err == io.EOF {    // 一直读到 EOF 为止
					break
				} else {
		...
		outputBuf0 := bufio.NewReader(stdout0)  // 一种更加便捷的方法，使用带缓冲的读取器从输出管道种读取数据
