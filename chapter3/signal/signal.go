...
func handleSignal() {
	...
	signal.Stop(sigRecv1)  // 发送信号
	close(sigRecv1)
	fmt.Printf("done. [sigRecv1]\n")
	wg.Wait() // 该方法会被一直阻塞，直到差量变为 0, 避免函数提前退出
}
...
func sendSignal() {
	...
	for _, pid := range pids {
		proc, err := os.FindProcess(pid) // 得到进程值
		if err != nil {
...
func runCmds(cmds []*exec.Cmd) ([]string, error) {
	...
	for _, cmd := range cmds {
		...
		if !first {
			...
			stdinBuf.Write(output)  // 前面的输出 作为后面命令的输入
			cmd.Stdin = &stdinBuf
		}
...
func getCmdPlaintext(cmd *exec.Cmd) string {
	...
	buf.WriteString(cmd.Path)  // 写入运行命令的路径
