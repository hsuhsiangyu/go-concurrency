...
func (s *segment) redistribute(pairTotal uint64, bucketSize uint64) (err error) {
	...
	s.pairRedistributor.UpdateThreshold(pairTotal, s.bucketsLen) // 先根据散列段中键-元素对的总数和散列桶的总数更新阈值
	bucketStatus := s.pairRedistributor.CheckBucketStatus(pairTotal, bucketSize) // 以阈值为依据判断当前散列桶的状态(过轻、过重或正常)
	newBuckets, changed := s.pairRedistributor.Redistribe(bucketStatus, s.buckets) // 在必要时对散列段中的所有键-元素进行再分布
	if changed {
